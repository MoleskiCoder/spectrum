...
protected:
	void playBuffer() override {
		...
		const auto success = ::SDL_PutAudioStreamData(m_stream.get(), m_buffer.data(), static_cast<int>(m_buffer.size()));
...
public:
	...
	void initialise() override {
		...
		m_buffer.resize(static_cast<uint64_t>(base::samplesPerFrame()));
		
		::SDL_LogInfo(SDL_LOG_CATEGORY_AUDIO, "Samples per frame: %f", base::samplesPerFrame());
		::SDL_LogInfo(SDL_LOG_CATEGORY_AUDIO, "Samples per frame (cast): %ld", static_cast<long>(base::samplesPerFrame()));
